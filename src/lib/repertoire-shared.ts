...
export function parseRepertoireCsv(text: string): CsvRow[] {
  ...
  const indices = {
    update_date: idx('update_date'),
    period: idx('period'),
    group_num: idx('group_num'),
  ...
  return lines.slice(1).map((line) => {
    ...
    return {
      update_date: get(cols, 'update_date'),
      period: get(cols, 'period'),
      group_num: get(cols, 'group_num'),
...
export function deriveFromLegacy(legacy: LegacyData): RepertoireWork[] {
